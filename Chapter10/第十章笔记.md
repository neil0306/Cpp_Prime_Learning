...
## 对象数组
对象数组: 就是创建由`多个对象`组成的数组, 数组的每个元素都是一个对象.
- 声明对象数组的方式与普通数组的声明方式相同, 只不过初始化的时候可以有特殊的操作.
  - `Stock mystuff[4];` 创建数组里的对象时调用的是`默认构造函数`.
  - 使用自定义构造函数: 
    ```cpp
        const int STKS = 4;
        // 声明对象数组时进行初始化, 并且调用了同一个自定义的构造函数
        Stock stocks[STKS] = {
        ...
        const int STK = 10;
        // 也可以在声明时调用不同的构造函数, 并且可以只初始化部分元素(没有显式声明的元素将使用默认构造函数进行构造)
        Stock stocks[STK] = {
            Stock("NanoSmart", 12, 20.0),
            Stock(),    // 默认
            Stock("Monolithic Obelisks", 130, 3.25),
...
int main(void)
{
    ...
    for(st = 1; st < STKS; st++){
        top = &top->topval(stocks[st]);     // 这里为了调用topval函数, 将之前的topval函数的返回值从 "const Stock" 改为 "const Stock&", 否则会因为该函数返回一个临时对象然后取临时对象的地址而报错(此时top将指向一个不合法的地址).
    }
...
## 类作用域
类里面定义的函数, 变量的作用域都只在类的内部. 我们无法直接从类的外部直接访问到类内部的变量(必须通过类的对象来访问).
- 在`类的声明`和`成员函数的定义里`可以直接使用类里面的其他函数, 不需要加"前缀" (比如`Stock::`这样的前缀).


### 在类中实现一个常量
- 注意: 直接在类里面写`const Months = 12;` 这样的语句是不合法的, 因为类的声明里只是一个中抽象的描述, 还没有真正在内存中创建任何对象, 而这里使用的语句本质上是需要`创建一个常量`, 这是需要使用内存空间的, 所以此时会报错.
- 为了实现常量的定义, 可以使用如下两个方法:
  - 1. 在类的声明中增加`枚举类型`, 此时这个枚举类型的`作用域是整个类`,  如: 
    ```cpp
        class Stock
        {
            private:
                // const Months = 12; 这个语句是不允许的!!
                enum {Months = 12};     // 用枚举类型实现常量的定义
                double costs[Months];   // 使用枚举类型
        }
    ```
    -  由于这种方式声明的枚举类型并不会创建类数据成员, 即所有对象中都不包含枚举, 并且 Months 本质上只是一个`符号名称/符号常量`, 因此不需要提供枚举名, 此时编译器只要遇到 Months 就会自动使用 数字 12 进行替换. 
  - 2. 使用`static关键字`:
    ```cpp
        class Stock
        {
            private:
                static const int Months = 12;   // 使用 static 关键字实现常量的定义
                double costs[Months];
        }
    ```
    - 这种方式相比于 `const Months = 12;` 这种写法唯一区别就是`增加了static关键字`, 它能有效是因为 static 修饰的变量在编译器进行编译时就马上被自动开辟内存空间进行存放的了, 此时不论这个类是否有创建对象, static 修饰的变量都已经存在于内存当中了, 因此这种方式是合法的. 也就是说, static 修饰的变量存放的位置 与 类的成员变量 存放的位置是不一样的, 这种变量也可以简单理解为"**全局变量**".
- 在第十二章会详细阐述`静态的类成员`.


## 抽象数据类型
抽象数据类型(Abstract Data Type, ADT) 可以理解为`定义一种通用型的类`, 它不限定某种数据类型(int, double什么的), 它是一种整合了数据和操作数据的函数的数据类型(有点像作文模板的意思).
- 比如`栈`, 它的特点就是`先入后出/后入先出`, 然后我们可以定义一个`通用的栈类`, 这个类可以用来存放任何类型的数据, 但是它的操作都是一样的, 比如`入栈`, `出栈`等等.
