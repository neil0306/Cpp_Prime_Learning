...
模板的友元分成3种:
- 非模板友元, 常规的那种友元函数函数
- 约束(bound)模板友元, 友元函数是模板函数, 当类模板实例化的时候, 友元函数也会被实例化, 并且使用与类模板相同的类型进行实例化
- 非约束(unbound)模板友元 
...
输出:
![](第十四章笔记_images/第十四章_非模板友元_例子输出.png)


#### 代码例子 -- 约束模板友元
- 把友元函数**声明在模板类之外**.
  - 在**类的外面声明了模板函数**, 然后再把这个**模板函数在类模板中声明为友元函数**
    - 注意友元函数不是类的成员函数, 需要把定义写在类的外面!
    ```cpp
    #include <iostream>
...
#### 非约束模板友元
- 把友元函数**声明在模板类内部**.
  - 首先在**类的内部声明了模板函数**, 并且再把这个**模板函数在类模板中声明为友元函数**
    - 由于友元函数不是类的成员函数, 所以都需要把函数定义写在类的外面.
    ```cpp
    #include <iostream>
    ...
    int main(void)
    {
        ...
        cout << "hfi1, hfi2: ";
        show2(hfi1, hfi2);  // 对应的函数定义为 void show2(ManyFriend<int>, ManyFriend<int>)

        cout << "hfdb, hfi2: ";
        show2(hfdb, hfi2);  // 对应的函数定义为 void show2(ManyFriend<double>, ManyFriend<int>)
...
### 模板的别名(C++11之后才有的)
其实就是使用`typedef`的方式给模板类起一个别名.
- 使用方法为:
    ```cpp
    template<typename T>  // 写成 template<class T> 也是可以的
    using arrtype = std::array<T, 12>;     // arrtype 是 std::array<T, 10> 的别名, 注意它仍然是一个模板, 而非具体的类

    arrtype<double> gallons;        // gallons 是 std::array<double, 12> 的对象
    arratype<int> days;             // days 是 std::array<int, 12> 的对象

    arrtype<std::string> months;    // months 是 std::array<std::string, 12> 的对象
    ```

在C++11中, `using =` 这种写法是通用的, 它也可以用于非类模板, 这样可以增加代码可读性.
- 如:
```cpp
typedef const char * pc1;
using pc2 = const char *;   // 与上一行等价
