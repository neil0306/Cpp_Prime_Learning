...
## 8.4 左值/右值引用(before 函数重载)
首先介绍一下C++的值的分类:
- 参考博客: https://www.cnblogs.com/zpcdbky/p/5275959.html
- C++11之前, 只有`左值(left value, lvalue)` 和 `右值(right value, rvalue)`两种,
- C++11之后, 出现了`左值(left value, lvalue)`, `纯右值(pure right value, pralue)`和`将亡值(expiring value, xvalue)`
  - C++11的表达式按值分如下图
    ![](第八章笔记_images/C++11的表达式按值分类.png)
    - C++11之后, 表达式按值划分分类时, 其结果必属于底层的三者之一, 即`左值`, `将亡值` 和`纯右值`
    - 左值和将亡值合并可称为`泛左值(general left value)`.
    - 纯右值和将亡值合并可称为`右值 (right value)`.

这些分类名词的具体含义:
- 左值: 能用 `&` 取到地址的**表达式**称为`左值表达式`.
- 纯右值, 需要满足下面两个条件之一: 
  - 要么是纯粹的`字面值/字面量 (literal)`, 如 `false, 1, 2, 3`.
    - 注意: 字符串属于字面量, 但是**字符串**是`左值`!!! 这是一个例外.
      - 例子就是 `char *p = "abc"; // 注意不是 char *p = &("abc");`
        - `"abc"`可以直接初始化指针p，p的值为字符串"abc"的首字符a的地址.
        - `&("abc")`被编译器编译为`const的指向数组的指针const char (*) [4]`（之所以是4，是因为编译器会在"abc"后自动加上一个'\0'），它不能初始化char *类型，即使是const char *也不行。
        ![](第八章笔记_images/字符串是左值_&(字符串)是右值.png)
  
  - 要么表达式的求值结果`相当于字面值`, 或者是一个`临时变量`.
    - 包含: `除字符串字面值以外`的字面值、返回非引用类型的函数调用、后置自增/自减运算符连接的表达式i++/i--、算术表达式（a+b、a&b、a<<b）、逻辑表达式（a&&b、a||b、~a）、比较表达式（a==b、a>=b、a<b）、取地址表达式（&a）等。
- 将亡值: 将亡值与右值引用息息相关, 它指的是下面的表达式:
  - **返回右值引用的函数**的调用表达式, 如 `std::move()`.
  - **转换为右值引用的转换函数**的调用表达式, 如 `static_cast<类型 &&>`.
...
- 使用场景: 移动语义
    ...
    // 移动语义: 使用 std::move(), 将左值干死, 并生成一个对应的右值 (本质上这个过程得到的是一个"将亡值").
    B aaa;                      // 我们先构造了一个左值 aaa
    B bbb = std::move(aaa);     // 调用B的移动构造函数构造出bbb
    
    /*
    针对代码 B bbb = std::move(aaa), 我们有如下分析过程:
        1. 用std::move(), 把 aaa 转换成右值, 此时aaa将被干掉, aaa会被直接释放, 同时生成出一个临时变量来存放aaa里面的所有内容
        2. 然后以临时变量作为 B 的移动构造函数的传入参数, 构造出 bbb, 注意, 移动构造函数入参是一个"右值", 而我们传入的恰好也是一个右值, 符合条件.

        注: 移动构造函数的触发还有一个"前提", 就是它的入参是一个"临时变量", 这个临时变量被构造函数使用之后就被销毁. 模仿视频里up主的理解方式来说就好比是 "爱因斯坦下一秒就要gg了, 我利用黑科技 std::move() 把爱因斯坦的脑子给偷过来了, 然后立马触发我的隐藏技能--移动构造函数, 直接构造了我自己的脑子, 此时, 我将一秒学会爱因斯坦的所有知识, 6得飞起~".  
    */

    // 还有一种写法也能触发 B 的移动构造函数:
    B ccc;                              // 我们先构造了一个左值 ccc
    B ddd = static_cast<B &&> ccc;      // 把ccc通过强制类型转换成右值, 然后使用移动构造函数构造出 ddd
    ```

    - (莫名其妙的)口诀: `移动语义不移动, 完美转发不完美`.
      - 移动: std::move 没有"移动", 真正的内容移动是在"移动构造函数"中进行的.
      - 转发: 到目前的阶段还没详细了解, to be continue...
  - 得到`将亡值(expiring value, xvalue)`的简单办法: 
    - C++11
      - 使用 `std::move(左值)`
      - 使用 `static_cast<类型 &&>(左值)`
    - C++17后才引入:
      - 临时量实质化 (暂未清楚是什么东西...)

  - To Be Continue... (书本的第18章好像要详细讲移动语义, 目前还没看到呢...)

### 额外补充的知识点1 (函数的返回值与临时变量)
C++规定当`函数返回的是非引用类型`时，函数`会创建临时对象`（temporary object），函数`返回的就是这个临时对象`。在求解表达式时，如果需要一个地方存储其运算结果，编译器会创建一个没有命名的对象，这就是临时对象。如下面的代码例子:
```cpp
/*
...
```
 - 小插曲: 如果我们使用`g++ 文件名.cpp`去编译这份代码, 得到的可执行文件是不会体现出我们想要看到的东西的, 因为`GCC 默认开启了返回值优化(Return Value Optimization, RVO)!!!`, 要体现出这些调用细节的话, 我们需要在编译时加上`-Wall -Wextra -pedantic -fno-elide-constructors`, 也就是:
    ```zsh
    g++ 文件名.cpp -Wall -Wextra -pedantic -fno-elide-constructors
    ```
    - 参考博客:  https://juejin.cn/post/7145704295353516063
        ![](第八章笔记_images/编译选项.png)
  ...
  - 得到运行结果:
    ```txt
        运行 test 1 函数....
        B的构造函数         // 构造主方法内的对象 t
        B的构造函数         // 构造 func2 内的局部对象b
        B的复制构造函数      // 将 func2 的局部对象复制到一个临时对象
        B的析构函数         // 析构局部对象 b
        B的赋值操作符       // 使用临时对象初始化 t
        B的析构函数         // 析构临时对象
        B的析构函数         // 析构对象t
        --------------------
        运行 test 2 函数....
        B的构造函数         // 构造 fun2 内的局部对象b  
        B的复制构造函数      // t 通过赋值构造函数进行初始化
        B的析构函数         // 析构 func2 内部对象b
        B的析构函数         // 析构 test2 的t
    ```
  - 对于`test1`: 由于 t 的初始化采用的是operator`=`操作符，**operator=** 要求必须使用一个**已经创建好了的对象对左值进行复制，所以此时必须先形成一个临时对象**，然后将临时对象赋值给 t.
  - 对于`test2`: 由于 t 是通过**复制构造函数**进行初始化的。复制构造函数初始化要求左值是一个**已有对象，而非创建好了的对象**，因此此时不需要创建一个临时对象.

### 额外补充知识点2
 - [x] From b站: https://www.bilibili.com/video/BV1Gg4y1p71w?p=3&vd_source=7cf7026bc2c23d0b0b88a3094e5ce55a
再次强调一下概念:
- 左值: 指向特定内存的**具名对象**, 简单来说就是`能取地址`的一定属于左值.
- 右值: **临时对象**, 指`除了字符串之外`的`字面量(literal)`.
  - 一个非常简单的例子: 对`表达式`或者`函数`进行取地址, 此时编译器会报错, 因为表达式和函数在执行完之后, 本质上是返回一个`临时对象`
    - 补充的代码例子1:
      ```cpp
      // 先定义一个全局变量, 用来观察"函数返回非引用类型而是变量的值时, 本质上是先产生一个临时变量" 的事实
      int gx = 10;        // gx 是一个具名对象(左值)
      ...
      int main(void)
      {
          int temp = get_x();   // 这个语句一共发生两次拷贝: 1) get_x 的return语句把全局变量 gx 拷贝到一个临时变量中; 2) 临时变量的值赋值到 temp.

          // 要验证这个小结论, 可以尝试对 get_x 取地址, 此时就能发现哪怕我们在函数中返回的是一个全局变量(不是返回全局变量的引用), 在函数返回时仍然产生了一个临时变量, 这导致取地址的操作编译失败. 
          &get_x();             // 这个语句在编译时会报错, 报错信息是 get_x() 是一个右值

      }

      /*
          这个例子说明:
          1. 函数在return阶段, 如果不是返回一个已有对象的引用时, 编译器会自动产生一个临时变量, 这是一个右值.  ---> 此时对函数表达时取地址是不行的
          2. 我们常用的 "调用函数然后赋值到新的变量" 这种语句至少要发生两次拷贝: 1) return的时候先把结果拷贝到一个临时变量中暂存, 然后释放掉函数体里能自动销毁的变量. 2) 然后再把临时变量赋值到 =号 的左侧, 这是第二次拷贝.
      */
      ```
    ...
    - 补充的例子2 (`&++x` 和 `&x++` 哪个能成功?):
      - 答案是: `&++x`.
    ![](第八章笔记_images/补充例子_&++和&++x哪个能成功.png)
      - `++x`是先执行完当前表达式, 然后才自增1, 所以`++x是一个左值`, `&++x`取的是x的地址
      - `x++`是先执行自增, 这相当于`先执行了一次表达式 x+1`, 此时表达式将返回一个`临时对象`, 这个对象属于`右值`, 故不能对它取地址. 
      - 也可以用下面的代码片段去理解:
        ```cpp
        // x++ 等效于执行了下面的函数
        int fun1(int &a)
        ...
        // ++x 等效于执行了下面的函数, 函数返回的时候没有生成临时变量, 而是把已经存在的对象a返回, 此时是一个左值
        int & fun2(int &a)      // 返回的是引用哦!
        {
            a = a + 1;          // a 自增
            return a;
        }
