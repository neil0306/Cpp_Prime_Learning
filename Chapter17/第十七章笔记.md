...
### 二进制文件
二进制文件的存储方式:
![](第十七章笔记_images/第十七章_浮点数的二进制表示与文本表示.png)
- 文本格式便于读取, 方便跨系统传输.
- 二进制对于数字数据来说毕竟精确, 不会出现转换误差或者舍入误差, 以二进制保存数字的数据更快(占用空间也较小).
  - 但是对于不同操作系统, 比如 64位 与 32位, 它们处理二进制数据的方式可能不同, 也就是说, 二进制数据在不同系统之间可能不兼容.

对于二进制文件, 我们读写需要使用`read()` 和 `write()` 方法, 而不是`<<` 和 `>>` 运算符.
...
1. setprecision()
2. cout << fixed;
3. fout.clear(); fin.clear();
4. fout.write((char *) &pl, sizeof pl);
5. fin.read((char *) &pl, sizeof pl);
```

#### 二进制文件的随机存储

这里指的是按照用户指定的某个位置进行存储.
代码例子 -- `random.cpp`
```cpp
#include <iostream>
#include <fstream>
...
知识点:
```cpp
1. fstream finout;      // 能同时管理读写的流对象
2. finout.seekg();      // 读文件时, 定位指针用 seekg() 
3. finout.seekp();      // 写文件时, 定位指针用 seekp()
4. if (finout.eof()){   
        finout.clear();      // 由于后面还要再操作文件, 所以先重置一下 eofbit  
    }
    /*
        eof() 方法用于判断是否读到文件的末尾, 如果返回true, 此时 finout对象 内部会设置一个 eofbit 标志位, 这时候 finout 在没有使用 clear() 重置状态之前将 无法再次读写该文件!!
    */
```
...
## 内核格式化
- iostream: 用于格式化输出(程序与标准输入输出设备之间的输出)
- fstream: 用于程序与文件之间的输入输出
- sstream: 用于程序与字符串之间的输入输出

使用内核格式化的目的:
- 可以轻松地`将数字转换为字符串`, 也可以`将字符串转成数字`
- `stringstream` 类可以使用 iostream 类的方法来管理存储字符串中的数据.
  - 类似fstream, stringstream 根据输出输入不同还可以细分出 istringstream 和 ostringstream.

代码例子 -- `stringstream_2.cpp`
```cpp
/*
    测试 stringstream 类, 把 record 文件里写入的 1~10 进行求和.
...
输出:
![](第十七章笔记_images/第十七章_代码例子输出_测试stringstream.png)

知识点:
```cpp
1. #include<sstream> // 要使用 stringstream, 必须引入这个头文件
2. 
    // 将捕获到的字符一个个存入 stringstream 对象中
    while(fin.get(ch)){
        ss << ch;         // 将读出来的字符放入 stringstream
