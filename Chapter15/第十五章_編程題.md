...
- 考察的知識點: 
  - 基類指針/引用 可以 指向派生類對象.
    - 由此, 可以只使用基類引用來`統一捕獲`**具有繼承關係的異常類**
  - 使用 `typeid` 運算符可以準確獲得基類指針所指向對象的類型
...
# 4.
- 考察的知識點與上一題相似:
  - 使用基類引用來`統一捕獲`**具有繼承關係的異常類**
  - 使用 typeid 對基類指針/引用進行進一步類型判斷
  - 使用了 `dynamic_cast<>()` 將基類指針轉換為派生類指針, 以實現派生類成員函數的調用.

- 只修改了 `use_sales.cpp` 文件
    ```cpp
    #include <iostream>
    #include "sales.h"
