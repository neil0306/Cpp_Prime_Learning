...
### 9.2.10 动态分配内存
- 使用 `new` 动态初始化一个内存块(要记得使用delete删除对应的变量, 防止出现`内存泄露`)
```cpp
int a = new int (6) // 在运行到这行语句时, 编译器才开始分配内存空间, 空间大小为一个 int 类型占用的大小, 然后赋初始值6.

struct where {double x; double y; double z;};
where  * one = new where {2.5, 3.5, 4.6};   // 运行到这里时, 动态创建一个where结构体指针变量one, 并赋值.
```

- 使用`定位(placement) new`动态初始化一个内存块:
  - 它与 `new` 的不同之处就在于, new 初始化的时候, 内存块是系统随机分配的, 但是`placement new`使用的内存块是**用户指定的**
    - 这种使用场景通常是`我们需要通过特定地址访问硬件, 或者在特定位置创建对象`.
    - 使用定位new的话, 需要包含头文件`#include <new>`
...
int main(void)
{
    ...
    // 释放 (pd2 和 pd4 不是常规的new得到的, 所以不能用delete, 使用的话, 在运行的时候会崩溃, 报错类似"对空指针进行free")
    delete[] pd1;
    delete[] pd3;
...
```
输出:
![](第九章笔记_images/new和placement_new_程序输出.png)

注意:
- **不能**对`placement new`创建的变量使用`delete`来释放, 因为placement new一般使用的地址都是`静态内存空间`, 比如代码例子中的buffer, 它其实是静态内存里的变量, 不能用delete来释放.




## 9.3 命名空间(namespace)
命名空间一般用来处理不同库之间 List, tree 等类(class) 可能出现的重名问题.

命名空间可以是`全局`的, 也可以`位于另一个命名空间中`, 但命名空间`不能位于代码块中`.
- 默认情况下, 命名空间的链接性为`外部链接性`(除非它引用了常量).

### 未限定的名称(unqualified name) 和 限定的名称(qualified name)
没有被装饰的名称成为未限定的名称(unqualified name); 包含名称空间的名称称为限定的名称(qualified name).
```cpp
// 假设已经定义过一个命名空间叫 Jack
Jack::pail = 12.34;    // 这种写法就是 qualified name

pail = 13.14;         // 这种写法就是 unqualifed name
```

### using声明 与 using编译指令
- using声明, 如:
  ```cpp
  using Jack::fetch;    // using 声明(fetch是一个函数)

  int main(void)
  {
    fetch();      // 前面有了using 声明, 这里就不需要再加上 Jack:: 了
  }
  ```

  - 特别需要注意的例子:
    ```cpp
    char fetch;   // 这是一个全局的 fetch

    int main()
    {
      using Jack::fetch;    // 使用命名空间的fetch作为"局部变量", 如果写在main的外面, 就是全局的了
      // double fetch;         // 这行语句如果执行的话会报错, 因为它与Jack里面的fetch冲突了
      cin >> fetch;         // 值会写入到 Jack::fetch 里, 而不是全局变量 fetch
      cin >> ::fetch;       // 写入全局变量 fetch, 因为使用了作用域解析运算符 :: (双冒号)
    }
    ```

- using编译指令
  ```cpp
  using namespace std;  // 这种就是 using 编译指令.
  int main(void){}; 
  ```
...
using Jack::pail;
using Jill::pail;
pal = 4;      // 这个语句就会产生二义性, 不知道到底pail对应那个命名空间的pail
```

注意 using 声明 和 using编译指令 之间的效果 & 区别:
```cpp
// 假如有这两个 namespace 定义
namespace Jack{
  double fetch;
...
char fetch;   // 全局变量fetch

int main(void)
{
  Jack::fetch = 13.14;    // 使用using声明语句, 它的作用域不是局部的, 不会覆盖掉全局变量fetch
  // double fetch;    // 如果这个语句替换掉Jack::fetch语句, 此时全局变量fetch会被覆盖掉, 注意这个语句会与Jack::fetch冲突
  cin >> ::fetch;         // 用 using声明语句时, 此时还能访问到全局变量fetch

  using namespace Jill;
  double Hill = 0.0;    // 这个语句不会发生冲突错误, 但是此时 Jill 里面的 Hill 会被隐藏掉, 就像局部变量隐藏了全局变量一样.
}
```
- 局部变量会隐藏掉`using namespace xxx`里的同名变量
- 局部变量`yy`会跟`using xxx::yy`产生冲突, 报错.
- `using xxx::yy` 比 `using namespace xxx` 更安全, 因为编译器会发出警告, 防止出现问题.
...
int main(void)
{
  std::cout << aaa::bbb::ddd << std::endl;    // 使用两次作用域解析符就可以访问到ddd.
}
```

例子2: 
```cpp
// using声明和using编译指令可以同时使用
namespace myth{
  using Jill::fetch;
...
### 未命名的命名空间
- 未命名的命名空间不能使用`using编译指令`或者`using声明指令`
- 未命名的命名空间的`潜在作用域`为**从声明位置到当前文件末尾**.
```cpp
namespace 
{           // 作用域起点
  int ice;
